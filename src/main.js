...
class LINE extends LineAPI {
  ...
  this.textMessage("0103",seq,operation.param2,1);
    ...
    async textMessage(textMessages, seq, param, lockt) {
        ...
        if(txt == 'tab:cancel' && this.sendCancel == 0 && isAdmin(seq.from)) {
           this.sendCancel = 1;
           this._sendMessage(seq, 'ᗩpakah Anda Yakin Untuk Cancel Semua Pendingan Dari Group Ini?\nJika Anda Yakin Silahkan Ketik [Yes] Dan Jika Anda Tidak Yakin Ketik [No] ( ´･ω･`)');
         }

        if(txt == 'yes' && this.sendCancel == 1 && isAdmin(seq.from)) {
                this.sendCancel = 0;
                let txt = await this._sendMessage(seq, 'Tunggu Sebentar ( ´･ω･`)');
                this.cancelAll(seq.to);
         }

         if(txt == 'no' && this.sendCancel == 1 && isStaff(seq.from)) {
                this.sendCancel = 0;
                this._sendMessage(seq, 'Cancel Pendingan Di Group Ini Telah Dibatalkan >_<');
        }

        if(txt == 'tab:cancel' && this.sendCancel == 0 && isStaff(seq.from)) {
           this.sendCancel = 1;
           this._sendMessage(seq, 'ᗩpakah Anda Yakin Untuk Cancel Semua Pendingan Dari Group Ini?\nJika Anda Yakin Silahkan Ketik [Yes] Dan Jika Anda Tidak Yakin Ketik [No] ( ´･ω･`)');
         }
...
        }
           ...
           if(cmd == 'Tab:Change:NameGroup' && isAdmin(seq.from)) {
           ...
           if(cmd == 'Tab:Change:NameGroup' && isStaff(seq.from)) {
                    let group = await this._getGroup(seq.to);
                    group.name = payload.replace('@','');
                    let change = await this._updateGroup(group);
                    this._sendMessage(seq, "Nama Group Telah Diganti Menjadi : "+group.name+"");
